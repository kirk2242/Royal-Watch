...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $description = trim($_POST['description']); // Added description field
    ...
    if (empty($messageType) || $messageType !== "error") {
        try {
            // Update query
            $update_stmt = $pdo->prepare("UPDATE products SET barcode=?, name=?, category=?, price=?, stock=?, brand=?, gender=?, image=?, description=?, updated_at=NOW() WHERE id=?");
            if ($update_stmt->execute([$barcode, $name, $category, $price, $stock, $brand, $gender, $imagePath, $description, $product_id])) {
