...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($messageType) || $messageType !== "error") {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO products (name, category, price, stock, barcode, brand, gender, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$name, $category, $price, $stock, $barcode, $brand, $gender, $imagePath])) {
